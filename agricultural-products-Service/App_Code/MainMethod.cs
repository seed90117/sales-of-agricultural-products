...
public class MainMethod
{
    ...
    {//Stage 這兩個欄位是否使用到，再考慮
        sql = "select ProductName PN, Price, Amount, PackagingDate PKD, Product.FarmID, Farm.FarmName FN, CertificationClassification.CertificationClassification CC, Certification.CertificationUnit CCU, Certification.CertificationUnit2 CCU2, Verification.VerificationUnit VU,Verification.ImgUrl, Product.Introduction PI, QRCode, Stage from Product"
    ...
    public string GetProductKey(string bigItem, string smallItem, string value, string CCID, string p, string h) // By Kevin Yen       by Huan-Chieh Chen 20170214
    {
        ...
        // 取得商品表單內資料
        sql = "select Product.ProductID, ProductName, Price, ProductImage.ImageUrl Image, CertificationClassification.CertificationClassification CC, Product.FarmID, Farm.FarmName from Product"
            + " INNER JOIN CertificationClassification ON Product.CertificationClassificationID = CertificationClassification.CertificationClassificationID"
            + " INNER JOIN Farm ON Product.FarmID = Farm.FarmID"
